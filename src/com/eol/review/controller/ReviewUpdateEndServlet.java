...
public class ReviewUpdateEndServlet extends HttpServlet {
	...
	//.쿨라이언트가 보낸 파일저장하기
		...
		r.setpName(mr.getParameter("pName"));
		try {
		r.setrNo(Integer.parseInt(mr.getParameter("rNo").trim()));
		}catch(NumberFormatException e) {
			e.printStackTrace();
		}
		
		String rTitle = r.getrTitle();
		String rContent = r.getrContent();
		String mImage = r.getmImage();
		int rNo = r.getrNo();
		
		int result=new ReviewService().updateReview(rNo, rTitle, rContent, mImage);
		
		String msg="";
		String loc="/review/reviewList";
		msg=result>0?"수정이 완료되었습니다.":"리뷰수정실패";
